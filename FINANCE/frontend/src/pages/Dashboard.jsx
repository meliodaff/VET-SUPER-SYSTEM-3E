...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      let surgeryFees = [];
      if (
        doctorSurgeryFeesRes?.data?.success &&
        Array.isArray(doctorSurgeryFeesRes.data.data)
      ) {
      ...
      if (salesMetricsRes?.data?.success && salesMetricsRes?.data?.data) {
        ...
        salesMetrics = {
          ...
          pending_amount: parseFloat(
            metrics.pending_amount || metrics.pending_invoices || 0
          ),
          pending_invoices: parseFloat(
            metrics.pending_invoices || metrics.pending_amount || 0
          ),
      ...
      setDashboardData({
        ...
        salesTrend: salesTrendRes?.data?.success
          ? salesTrendRes?.data?.data || []
          : [],
      ...
      setError(
        error.response?.data?.message ||
          error.message ||
          "Failed to load dashboard data. Please check if XAMPP is running."
      );
    } finally {
      setLoading(false);
    }
